...
vector<double>& padd, double Tstart, double clsizef)
{
...
int main()
{
	...
	for (int k = 300; k < 700; k++) {
		...
		for (int h = 0; h < 3; h++) {
			wolff_cycle(size, 0.005 * k, Mag, mag_sus, Mag2, Mag4, Ene, sp_heat, near, K, J, padd, Tstart, clsizef);
			Fileout << size << " " << 0.005 * k << " " << Mag << " " << Mag2 << " " << Mag4 
