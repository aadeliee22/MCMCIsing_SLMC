...
vector < vector <double> >& na)
{
	...
	int sp = 0, sh = 0, n2 = 0, n3 = 0; double point = 0;
	while (1) {
		for (int k = 0; k < 4; k++){
			point = na[i][k];
			n2 = search[na[point][4]] + search[na[point][5]] + search[na[point][6]] + search[na[point][7]];
			n3 = search[na[point][8]] + search[na[point][9]] + search[na[point][10]] + search[na[point][11]];
			if (v[point] == oldspin && dis(gen) < padd[5*n2+n3]) { 
				sh++; 
...
int main()
{
	...
	ofstream Fileout; 
	Fileout.open("cluster_search_3.txt");
	cout << "File search3 open: " << endl;
	...
	for (int m = 0; m < 5; m++){
		...
		for (int n = 0; n < 7; n++){
			...
			for (int k = 200; k < 700; k++) {
				temp = 0.005 * k;
				for (int i = 0; i < 5; i++){ 
					for (int j = 0; j < 5; j++){ padd[5*i + j] = 1 - exp(-2 * (J[1] + i * J[2] + j * J[3]) / temp); }
				}
				for (int h = 0; h < 2; h++) {
